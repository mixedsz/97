...
-- Helper function to find a weapon part in any location
function FindWeaponPart(item, price)
    for _, location in ipairs(Config.Locations) do
        if location.availableParts then
            for _, part in ipairs(location.availableParts) do
...
AddEventHandler('ammu:buyWeaponPart', function(item, price)
    ...
    if not IsJobAllowed(xPlayer.job.name) then
        print('[ammu:buyWeaponPart] Job not allowed for player: ' .. xPlayer.job.name)
        return
    end

    -- Check if item exists in any location's availableParts
    local validItem = FindWeaponPart(item, price)

    if not validItem then
        print('[ammu:buyWeaponPart] Invalid item or price: ' .. item .. ' @ $' .. price)
        TriggerClientEvent('ox_lib:notify', source, {
            title = 'Error',
            description = 'Invalid item purchase',
            type = 'error'
        })
        return
    end
    ...
    if xPlayer.getMoney() >= price then
        ...
        -- Try adding item with multiple methods
        local itemAdded = false
        
        -- Method 1: Try standard addInventoryItem
        local success, result = pcall(function()
            return xPlayer.addInventoryItem(item, 1)
        end)
        
        if success and result then
            itemAdded = true
            print('[ammu:buyWeaponPart] Successfully added item using addInventoryItem: ' .. item)
        end
        
        -- Method 2: Try ox_inventory export if method 1 failed
        if not itemAdded then
            local oxSuccess, oxResult = pcall(function()
            ...
            if oxSuccess and oxResult then
                itemAdded = true
                print('[ammu:buyWeaponPart] Successfully added item using ox_inventory: ' .. item)
            end
        end
        
        -- Method 3: Try qs-inventory if previous methods failed
        if not itemAdded then
            local qsSuccess, qsResult = pcall(function()
            ...
            if qsSuccess and qsResult then
                itemAdded = true
                print('[ammu:buyWeaponPart] Successfully added item using qs-inventory: ' .. item)
            end
        end
        ...
        if itemAdded then
            TriggerClientEvent('ox_lib:notify', source, {
                ...
                description = 'You purchased ' .. item .. ' for $' .. price,
                type = 'success'
            })
        else
            -- Refund money if item couldn't be added
            xPlayer.addMoney(price)
            print('[ammu:buyWeaponPart] FAILED to add item, refunding money: ' .. item)
            TriggerClientEvent('ox_lib:notify', source, {
                title = 'Error',
                description = 'Could not add item to inventory. Money refunded.',
                type = 'error'
            })
        end
    else
